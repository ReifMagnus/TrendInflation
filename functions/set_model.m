...
if n_dRWd ~= 0
    sigtau2_ub = .005;                 % upper bound for variance of gdp trend
    n_grid     = 500;
end
...
real_trends    = n_real + n_dRWd;

infl_trend   = 1;
infl_gap     = pc_q*n_infl;
...
shocks_infl_trend  = ones(1,1);
shocks_infl_gap    = repmat([1,zeros(pc_q-1)],1,n_infl);
...
shocks_to_Q = logical([shock_gap shock_trend shocks_costp_trend shocks_costp_gap shocks_infl_trend shocks_infl_gap shocks_exp_cycle shocks_costp_cycle]);
...
K = s + real_trends + costp_gap + costp_trend + infl_trend + infl_gap + n_expcycle + n_costpcycle;          % length of state vector

fac_ix           = 1:s;                                           % factor
real_trend_ix    = find([zeros(1,s) [1 zeros(n_dRWd,1)] ones(1,n_real-1)]);
costp_trend_ix   = (s + real_trends + 1)*n_costp(n_costp~=0);          % cost-push trend
costp_gap_ix     = (costp_trend_ix + (1:costp_gap))*n_costp(n_costp~=0);      % cost-push gap
infl_trend_ix    = s + real_trends + costp_gap + costp_trend + 1;
infl_gap_ix      = s + real_trends + costp_gap + costp_trend + infl_trend + (1:n_infl + (pc_q - 1)*n_infl);
infl_expcyc_ix   = s + real_trends + costp_gap + costp_trend + infl_trend + n_infl + (pc_q - 1)*n_infl + (1:n_expcycle);
costp_cycle_ix   = s + real_trends + costp_gap + costp_trend + infl_trend + n_infl + (pc_q - 1)*n_infl + n_expcycle + (1:n_costpcycle);
