function [xdraw, loglik] = disturbance_smoother_sv(y,A,H,R,B,F,Q,x0,cSig0,opt_output)
...
for t = T-1:-1:1
    ...
    if length(iobs) == nobs
        rrr(:,t) = H(:,:,t+1)'*FFFinv(:,:,t+1)*vvv(:,t+1) + (F(:,:,t) - KKK(:,:,t+1)*H(:,:,t+1))'*rrr(:,t+1);
    elseif ~isempty(iobs)
        Ftp1inv  = FFFinv(iobs,iobs,t+1);
        ZZtp1    = H(iobs,:,t+1);
        vtp1     = vvv(iobs,t+1);
        Ktp1     = KKK(:,iobs,t+1);
        rrr(:,t) = ZZtp1'*Ftp1inv*vtp1 + (F(:,:,t+1) - Ktp1*ZZtp1)'*rrr(:,t+1);
    else        
        rrr(:,t) = F(:,:,t+1)'*rrr(:,t+1); % backward recursion with no observables
    end
end

% one more iteration to get r0
iobs = find(~isnan(yy(:,1)));
r0   = H(iobs,:,1)'*FFFinv(iobs,iobs,1)*vvv(iobs,1) + (F(:,:,1) - KKK(:,iobs,1)*H(iobs,:,1))'*rrr(:,1);     % Durbin/Koopmann (2002), eq.5

xdraw = nan(nstates,T);                                                       % allocate space for smoothed states

% forwards recursion to compute smoothed states from r, Durbin/Koopmann (2002), eq.8
xdraw(:,1) = x0 + cSig0*cSig0'*r0;                                            % initialize the forward recursion
for t = 2:T
    xdraw(:,t) = B(:,t) + F(:,:,t)*xdraw(:,t-1) + Q(:,:,t-1)*Q(:,:,t-1)'*(rrr(:,t-1));
end
