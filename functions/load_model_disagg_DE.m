...
spec.indicators = cell2table({'INFEXP_LR'          ,'Exp'        ,'RW'          ,[]           ,'Error'      ,.5;    
                              'INFEXP_SR'          ,'Exp'        ,'RW'          ,[]           ,'Error'      ,.5;         
                              'FOOD'               ,'Infl'       ,'RW'          ,[1 1]        ,'Error'      ,.5;
                              'ALCOHOL'            ,'Infl'       ,'RW'          ,[1 1]        ,'Error'      ,.5;    
                              ...
                              'COMMUNICATION'      ,'Infl'       ,'RW'          ,[1 1]        ,'Error'      ,.5;                                                                
                              'RECREATION'         ,'Infl'       ,'RW'          ,[1 1]        ,'Error'      ,.5;    
                              'EDUCATION'          ,'Infl'       ,'RW'          ,[1 1]        ,'Error'      ,.5;    
                              'FOODSERVICES'       ,'Infl'       ,'RW'          ,[1 1]        ,'Error'      ,.5;                                  
                              'OTHERSERVICES'      ,'Infl'       ,'RW'          ,[1 1]        ,'Error'      ,.5; 
                              'GDP'                ,'Real'       ,'dRWd'         ,0            ,'Exact'      ,.5;     
...
spec.prior.loading.gap.variance   = .1^2;
spec.prior.loading.trend.variance = .1^2;
...
spec.prior.pc.output_gap.mean(2) = 0;                                                                     % prior on first lag of output gap
...
spec.prior.pc.output_gap.variance(2) = .1^2;%infl_to_gdp(1)/2;                                       % first lag of output gap
spec.prior.pc.infl_gap.variance      = ones(spec.PC.AR_lags,1)*.1^2;
spec.prior.pc.infl_gap.variance(1)   = .2^2;
...
spec.prior.shocks.scale.real_trends     = 0.09/4;
...
spec.prior.shocks.scale.inf_exp_trend   = 0.09;
