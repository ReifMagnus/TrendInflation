function [H,F,R,Q,A,B] = get_state_space_disagg(Hraw,Fraw,loadings,ab,a_g,a_p,a_c,f_z,c,d,f_d,...
    phi,psi,zeta,h_g,h_rho,eps,eps_shocks,eta,eta_shocks,n,n_real,k,s,pp,L,Lix,sspace)
...
H(sspace.real_ix,sspace.fac_ix)             = loadings(1:n_real,:);
H(sspace.infl_ix,sspace.infl_trend_ix(1))   = ab(1,sspace.infl_ix);
H(sspace.infl_ix,sspace.infl_gap_ix)        = ab(2,sspace.infl_ix);
H(sspace.exp_ix,sspace.exp_ix)              = ab(:,sspace.exp_ix);
% if tvp
%     H                             = repmat(H,1,1,T);
...
%% transition matrix
F                                                           = Fraw;
F(sspace.fac_ix(1),sspace.fac_ix(1:s-1))                    = phi;
F(sspace.costp_trend_ix,sspace.costp_trend_ix)              = f_d(2);
% F(sspace.infl_trend_ix(1),sspace.infl_trend_ix(1))          = f_z(2);

F(sspace.infl_gap_ix(1),sspace.fac_ix(1:length(a_g)))      = a_g;
...
% B(sspace.infl_trend_ix(1)) = f_z(1);

%% observation error
R                  = repmat(eye(n),1,1,T);
eps(eps==0)        = 10^-6;
R(find(R))         = eps;
% R(:,~eps_shocks,:) = [];
