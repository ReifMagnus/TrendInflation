...
shocks_to_Q = logical([shocks_infl_gap shocks_infl_trend shock_gap shock_trend shocks_costp_trend shocks_costp_gap shocks_exp_cycle shocks_costp_cycle]);
...
K = s + real_trends + costp_gap + costp_trend + infl_trends + infl_gap_c + n_expcycle + n_costpcycle;          % length of state vector

infl_trend_ix    = [1 2 + (1:infl_trends-1)];
infl_gap_ix      = 2;
fac_ix           = 1 + infl_trends + (1:s);                                           % factor
real_trend_ix    = 1 + infl_trends + s + (1:n_real);
costp_trend_ix   = 1 + infl_trends + s + n_real + (1:costp_trend)*n_costp(n_costp~=0);          % cost-push trend
costp_gap_ix     = 1 + infl_trends + s + n_real + +costp_trend + (1:costp_gap)*n_costp(n_costp~=0);          % cost-push gap
infl_expcyc_ix   = 1 + infl_trends + s + n_real + +costp_trend + costp_gap + (1:n_expcycle);
costp_cycle_ix   = 1 + infl_trends + s + n_real + +costp_trend + costp_gap + n_expcycle + (1:n_costpcycle);;
