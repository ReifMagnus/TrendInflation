...
ab(1,exp_ix+1:end)    = a0 + scale_var*chol(1/iVab0(1,1))*randn(n_infl,1);
ab(2,exp_ix+1:end)    = b0 + scale_var*chol(1/iVab0(2,2))*randn(n_infl,1);
ab(:,exp_ix)          = [1; 0];
ab(exp_ix+1,exp_ix+1) = 1;
...
Sig0_infl_trend     = diag([1 sqrt(sigma_nu(infl_ix))]);    % fix initial condition of common trend to 0
...
cSig0 = blkdiag(Sig0_infl_gap,Sig0_infl_trend,Sig0_output_gap,Sig0_realtrends,Sig0_costp_trend,Sig0_costp_cycle,Sig0_infl_exp_cyc,Sig0_costp_cyc);
Sig0  = cSig0*cSig0';
...
if SV == 1
    ...
    htilde_obs                = mvnrnd(h0_obs',diag(sig_obs(1:n_infl + n_exp)),T);                % initial vola
    sig_obs                   = repmat(sig_obs,1,T);
    sig_obs(1:n_infl+n_exp,:) = exp(htilde_obs)';
    
    % transition errors
    h0_trans     = zeros(1,infl_trends + infl_gap_c + 1);    
    omegah_trans = sqrt(ones(1,length(h0_trans))*.2);                                                                              % std deviation of rw process
    htilde_trans = mvnrnd(h0_trans',diag(sig_trans([infl_trend_ix(1) infl_gap_ix infl_trend_ix(2:end) fac_ix(1)])),T);
    sig_trans    = repmat(sig_trans,1,T);
    sig_trans([infl_trend_ix(1) infl_gap_ix infl_trend_ix(2:end) fac_ix(1)],:) = exp(htilde_trans)';    
