...
spec.prior.pc_head.output_gap.variance = ones(spec.PC.OutputGap_lags,1)*.001^2;                                % tightly centered around zero
spec.prior.pc_head.infl_gap.variance   = ones(spec.PC.AR_lags,1)*.1^2;
spec.prior.pc_head.costp_gap.variance  = ones(spec.PC.CostPush_lags,1)*.1^2;                                                          % prior on inflation gap
