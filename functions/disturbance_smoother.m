function [xdraw, loglik] = disturbance_smoother(y,A,H,R,B,F,Q,x0,cSig0,opt_output)
...
if opt_output == 2
    ...
    for t = 1:T
        ...
        xplus(:,t+1) = F(:,:,t)*xplus(:,t) + Q*randn(nstatesshocks,1);
    end
    xplus(:,end) = [];
    yy = y - yplus;
else
...
for t = T-1:-1:1
    ...
    if length(iobs) == nobs
        rrr(:,t) = H(:,:,t)'*FFFinv(:,:,t+1)*vvv(:,t+1) + (F(:,:,t) - KKK(:,:,t+1)*H(:,:,t))'*rrr(:,t+1);
    elseif ~isempty(iobs)
        Ftp1inv  = FFFinv(iobs,iobs,t+1);
        ZZtp1    = H(iobs,:,t);
        vtp1     = vvv(iobs,t+1);
        Ktp1     = KKK(:,iobs,t+1);
        rrr(:,t) = ZZtp1'*Ftp1inv*vtp1 + (F(:,:,t) - Ktp1*ZZtp1)'*rrr(:,t+1);
    else        
...
r0   = H(iobs,:,1)'*FFFinv(iobs,iobs,1)*vvv(iobs,1) + (F(:,:,1) - KKK(:,iobs,1)*H(iobs,:,1))'*rrr(:,1);
...
for t = 2:T
    xdraw(:,t) = B(:,t) + F(:,:,t)*xdraw(:,t-1) + RQR*(rrr(:,t-1));
end
